...
import { DokkuAuth, createTestApp, destroyTestApp } from '../helpers/dokku';
...
describe('Frontend Service (Authelia)', () => {
  ...
  it('should show frontend status as running', async () => {
    const result = await auth.exec(`auth:frontend:status ${serviceName}`);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('running');
  });
  ...
  it('should show frontend logs', async () => {
    const result = await auth.exec(`auth:frontend:logs ${serviceName} --tail 5`);
    expect(result.exitCode).toBe(0);
  ...
  it('should protect an app', async () => {
    const result = await auth.exec(`auth:frontend:protect ${serviceName} ${testAppName}`);
    expect(result.exitCode).toBe(0);
  ...
  it('should unprotect an app', async () => {
    const result = await auth.exec(`auth:frontend:unprotect ${serviceName} ${testAppName}`);
    expect(result.exitCode).toBe(0);
...
describe('Frontend Service - Adopt Existing App', () => {
  ...
  afterAll(async () => {
    await auth.exec(`auth:frontend:destroy ${serviceName} -f --keep-app`).catch(() => {});
    await destroyTestApp(appName).catch(() => {});
  }, 60000);

  it('should adopt an existing Dokku app', async () => {
    const result = await auth.exec(`auth:frontend:create ${serviceName} --app ${appName}`);
    expect(result.exitCode).toBe(0);
  ...
  it('should fail to adopt a non-existent app', async () => {
    ...
    const result = await auth.exec(`auth:frontend:create ${bogusService} --app non-existent-app-xyz`);
    expect(result.exitCode).not.toBe(0);
    // Clean up the service dir that was created before the error
    await auth.exec(`auth:frontend:destroy ${bogusService} -f`).catch(() => {});
  });

  it('should destroy with --keep-app preserving the Dokku app', async () => {
    const result = await auth.exec(`auth:frontend:destroy ${serviceName} -f --keep-app`);
    expect(result.exitCode).toBe(0);
