#!/usr/bin/env bash
# ============================================================================
# WARNING: UNTESTED INTEGRATION
# This integration preset has not been validated with E2E tests.
# It may not work correctly with the current plugin version.
# Use at your own risk and please report issues.
# ============================================================================

# Lidarr integration preset
# https://lidarr.audio

PRESET_NAME="lidarr"
PRESET_DESCRIPTION="Lidarr music collection manager"
PRESET_SCOPES=""
PRESET_REQUIRE_PKCE=false
PRESET_PUBLIC=false
PRESET_OIDC_SUPPORTED=false
PRESET_LDAP_SUPPORTED=false
PRESET_PROXY_AUTH=true

# No OIDC redirect
preset_redirect_uri() {
  echo ""
}

# No OIDC env vars
preset_env_vars() {
  echo ""
}

# Post-integration instructions
preset_instructions() {
  local SERVICE="$1"
  local APP="$2"
  local CLIENT_ID="$3"
  local AUTH_DOMAIN="$4"

  cat <<EOF

Lidarr Setup Instructions:
==========================

Lidarr does not support OIDC or LDAP natively.
Use Authelia forward auth to protect it.

Protect the App:
----------------
dokku auth:protect $APP \\
  --service $SERVICE \\
  --bypass-path "/api/*" \\
  --bypass-path "/feed/*"

Bypass paths allow:
- /api/* - API access for automation tools
- /feed/* - RSS feeds

API Key Authentication:
-----------------------
Find your API key in: Settings > General > Security

For external apps, use:
  https://<domain>/api/v1/...?apikey=<your-api-key>

Note: Lidarr uses API v1, not v3 like Radarr/Sonarr.

Header Authentication (Optional):
---------------------------------
Lidarr supports external authentication via headers.
In Settings > General > Authentication:
- Authentication: External
- Authentication Required: Disabled for API

Additional Bypass Paths (if needed):
------------------------------------
--bypass-path "/ping"       # Health checks
--bypass-path "/initialize" # First-run setup

EOF
}

# No LDAP support
preset_ldap_config() {
  cat <<EOF
Lidarr LDAP Support:
====================

Lidarr does not support LDAP authentication.
Use forward auth (dokku auth:protect) instead.

EOF
}
