...
provider_create_container() {
  ...
  "$DOKKU_BIN" storage:mount "$APP_NAME" "$DATA_DIR/slapd:/var/lib/ldap" 2>/dev/null || true
  "$DOKKU_BIN" storage:mount "$APP_NAME" "$DATA_DIR/config:/etc/ldap/slapd.d" 2>/dev/null || true
